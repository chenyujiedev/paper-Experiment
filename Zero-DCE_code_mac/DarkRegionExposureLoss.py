# -*- coding: utf-8 -*-
# @Time    : 2026/3/14 20:53
# @Author  : 陈宇杰（CHEN YUJIE）
# @File    : DarkRegionExposureLoss.py
...
class L_exp(nn.Module):
    ...
    def forward(self, x):
        ...
        mean = self.pool(x)
        target = torch.tensor([self.mean_val], device=x.device)
...
class DarkRegionExposureLoss(nn.Module):
    ...
    def forward(self, input_img, enhanced_img):
        # 计算输入图像亮度
        input_lum = 0.299 * input_img[:, 0:1, :, :] + \
                    0.587 * input_img[:, 1:2, :, :] + \
                    0.114 * input_img[:, 2:3, :, :]

        # 计算增强图像亮度
        enhanced_lum = 0.299 * enhanced_img[:, 0:1, :, :] + \
                       0.587 * enhanced_img[:, 1:2, :, :] + \
                       0.114 * enhanced_img[:, 2:3, :, :]

        # 暗区权重：越暗越接近 1，越亮越接近 0
        dark_mask = torch.clamp(1.0 - input_lum, 0.0, 1.0)

        # 做 patch-level 平均，保持和原始 L_exp 一致的风格
