...
class HighlightPreservationLoss(nn.Module):
    """
    更早触发、但更平滑的高光保护
    目标：
    1. 比现在更早开始保护高亮区域
    2. 只惩罚增强后比原图更亮的部分
    3. 用更平滑的线性惩罚，减少训练后期把亮度顺序拉坏
    """
    def __init__(self, patch_size=8, tau=0.86, beta=1.5, margin=0.025):
        super(HighlightPreservationLoss, self).__init__()
        self.pool = nn.AvgPool2d(patch_size)
        self.tau = tau
        self.beta = beta
        self.margin = margin
    ...
    def forward(self, input_img, enhanced_img):
        ...
        mean_input = self.pool(input_lum)
        mean_enhanced = self.pool(enhanced_lum)

        # 比原来更早保护高亮 patch
        highlight_mask = torch.clamp(
            (mean_input - self.tau) / (1.0 - self.tau + 1e-6),
            min=0.0,
            max=1.0
        )
        highlight_mask = torch.pow(highlight_mask, self.beta)

        # 只惩罚增强后比原图更亮的部分
        overflow = F.relu(mean_enhanced - mean_input - self.margin)

        # 用线性惩罚，避免少数极亮 patch 梯度过大
        loss = torch.sum(highlight_mask * overflow) / (
            torch.sum(highlight_mask) + 1e-6
        )
        return loss